...
EEGColumns = DataColumns[0:25] # Brain Signals
EMGColumns = DataColumns[25:30] # Muscle signals
...
order = 2
f =19
def Label_Rolling_Window(label,window_size = 1500, stride=150):
    ...
    for i in range(0, len(time_series)-window_size, stride):
        ...
        lables = 0 if np.array(window_data).sum()<1200 else 1
...
def make_scalogram(data, freq, window_size,fnl_downsapl=20):
    ...
    data = Label_Rolling_Window(data)
...
def Datapreprocess(data):
    ...
    #fs = 200#int(np.floor(np.mean(1. / np.diff(t))))
    lowcut = 0.01
    highcut = 50#100
...
def DatapreprocessY(data):
    ...
    #fs = 200#int(np.floor(np.mean(1. / np.diff(t))))
    highcut = 20
    X_fltr =butter_lowpass_filter(data, highcut, fs=fs, order=7)
    #X_fltr = bandpass_filter(data, highcut, inplace=False,
    #avg_x = np.average(X_fltr, axis=1)
    #X = X_fltr - avg_x[:, None] # re-referencing the signal
    return X_fltr
